...
import { NgoListFilter, NgoListSearch, NgoListSort, Order } from "@/lib/types";
export interface NgoListFilterDialogProps {
  sortOnComplete: (itemName: NgoListSort) => void;
  searchFilterChanged: (itemName: NgoListSearch) => void;
  orderOnComplete: (itemName: Order) => void;
  dateOnComplete: (selectedDates: DateObject[]) => void;
  filters: NgoListFilter;
  filtersShowData: {
    sort: IShowData[];
    order: IShowData[];
    search: IShowData[];
...
export default function FiltersDialog(props: NgoListFilterDialogProps) {
  const {
    sortOnComplete,
    searchFilterChanged,
    orderOnComplete,
  ...
  const handleSort = (itemName: string) => {
    sortOnComplete(itemName as NgoListSort);
    modelOnRequestHide();
  };
  const handleSearch = (itemName: string) => {
    searchFilterChanged(itemName as NgoListSearch);
    modelOnRequestHide();
  };
